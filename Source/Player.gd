...
#Debug
onready var Selfstates = $States

func _physics_process(delta: float) -> void:
	var _is_free: = _is_attacking == false and _is_hit == false and _is_dodging == false and _is_spot_dodge == false
	...
	# Movement
	var _is_falling: = _velocity.y >= 0.0 and not is_on_floor() and not _is_spot_dodge
	...
	if _is_spot_dodge:
		...
		if _is_fast_falling:
			...
			_velocity.y = clamp(_velocity.y, _velocity.y, max_fast_fall_speed)
	elif _is_knockback:
		...
		_velocity.y += fast_fall_strength
	elif _is_idling or _is_running or is_on_wall():
		_jumps_made = 0
	...
	var _spot_dodge: bool = _dodge and not (Input.is_action_pressed("move_right_%s" % id) and Input.is_action_pressed("move_left_%s" % id))
...
#		print_debug(_attack_side)
	...
	if _spot_dodge:
		DebugState("Spot_Dodge")
		_is_spot_dodge = true
		_animation.current_animation = "spot_dodge"
		yield(_animation, "animation_finished")
		_is_spot_dodge = false
...
func DebugState(State: String):
	Selfstates.text = State
